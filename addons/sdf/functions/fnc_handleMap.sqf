...
_ctrlMap ctrlMapAnimAdd [0,(ctrlMapScale _ctrlMap) * 2,_mapPos];
...
[_ctrlMap,"MouseMoving",{
	...
	private _current = _ctrlMap ctrlMapScreenToWorld getMousePosition;
	_current set [2,0];
	private _markers = _ctrlMap getVariable QGVAR(markers);
	...
	if (_current inArea _marker) then {
		if (_ctrlMap getVariable QGVAR(areaSpecial)) then {
			_ctrlMap ctrlSetTooltip format [localize "STR_SDF_MapToolTipSpecial",_ctrlMap getVariable QGVAR(areaSpecialTip)];
		} else {
			_ctrlMap ctrlSetTooltip localize "STR_SDF_MapToolTip";
		};
	} else {
